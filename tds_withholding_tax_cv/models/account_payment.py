...
class account_payment(models.Model):
    ...
    tds_invoice_amount = fields.Float("Monto de la Factura a Pagar")
    ...
    @api.onchange('tds', 'tds_tax_id', 'tds_invoice_amount')
    @api.depends('tds', 'tds_tax_id', 'tds_invoice_amount')
    def onchange_tds(self):
        for payment in self:
            if payment.tds and payment.tds_tax_id and payment.tds_invoice_amount:
                applicable = True
                ...
                if self.payment_type == 'inbound' and applicable:
                    payment.tds_amt = -(payment.tds_tax_id.amount * payment.tds_invoice_amount / 100)
    ...
    def _create_payment_entry(self, amount):
        ...
        if self.tds and self.tds_tax_id and self.tds_amt and applicable:
            ...
            print("AMOUNTTETTETETTET % r "  % amount )
            ...
            print ([move, debit, credit, amount_currency])
            ...
            if payment_difference_handling == 'reconcile' and payment_difference:
                ...
                debit_wo, credit_wo, amount_currency_wo, currency_id = aml_obj.with_context(date=self.payment_date)._compute_amount_fields(payment_difference, self.currency_id, self.company_id.currency_id)



                print("DEBUGGSGGSGSGGSG % r"  % [debit_wo, credit_wo, amount_currency_wo])
                ...
                print("COUNTERPART %r " % counterpart_aml)
